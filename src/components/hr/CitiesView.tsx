...
export function CitiesView({ embedded = false }: { embedded?: boolean }) {
  const { can, locale } = useAuth();
  const t = (key: string) => translate(locale, key);
  const [tab, setTab] = useState<Tab>("operating-cities");
  const [globals, setGlobals] = useState<HrRow[]>([]);
  const [operating, setOperating] = useState<HrRow[]>([]);
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-[var(--muted)]">
              {locale === "en"
                ? "Manage global cities and activate operating locations."
            ...
            {tab === "global-cities" ? (
              ...
              <>
                <label className="grid gap-2 font-bold">
                  ...
                  >
                    <option value="">
                      {locale === "en" ? "Select City" : "اختر مدينة"}
                    </option>
                    {globals.map((city) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <span className="flex items-center gap-2 font-black">
                  ...
                  {text(
                    locale === "en"
                      ? city.nameEn || city.nameAr || city.code
              ...
              <p className="mt-2 text-sm text-[var(--muted)]">
                {text(locale === "en" ? city.nameAr : city.nameEn)} ·{" "}
                {text(city.code)}
              </p>
